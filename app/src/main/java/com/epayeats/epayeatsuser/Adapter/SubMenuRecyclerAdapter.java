...
public class SubMenuRecyclerAdapter extends RecyclerView.Adapter<SubMenuRecyclerAdapter.ImageHolder>
{
    Context context;
    List<MenuModel> menuModel;

    public SubMenuRecyclerAdapter(Context context, List<MenuModel> menuModel) {
    ...
    @NonNull
    @Override
    public SubMenuRecyclerAdapter.ImageHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {

        View view = LayoutInflater.from(context).inflate(R.layout.sub_menu_recycler, parent, false);
        return new SubMenuRecyclerAdapter.ImageHolder(view);

    }

    @Override
    public void onBindViewHolder(@NonNull SubMenuRecyclerAdapter.ImageHolder holder, int position)
    {
        ...
        holder.addtocartlayout1.setOnClickListener(new View.OnClickListener() {
            ...
            {
                HomeSubMenuCatagory_Activity.subMenuAddtoCartInterface.addtocartmainmenu(position, currentUpload.getMenuID(), currentUpload.getMenuName(), currentUpload.getImage1(), currentUpload.getMenuMainCatagorey(), currentUpload.getMenuMainCatagoryID(), currentUpload.getMenuSubCatagorey(), currentUpload.getMenuSubCatagoreyID(), currentUpload.getMenuLocalAdminID(), currentUpload.getMenuDescription(), currentUpload.getMenuOfferPrice(), currentUpload.getMenuSellingPrice(), currentUpload.getMenuActualPrice(), currentUpload.getMenuOpenTime(), currentUpload.getMenuCloseTime(), currentUpload.getMenuOnorOff(), currentUpload.getMenuUnit(), currentUpload.getMenuApprovel(), currentUpload.getRestID(), currentUpload.getRestName());
            }
        });
